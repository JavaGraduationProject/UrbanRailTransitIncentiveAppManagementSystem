<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.wanchengtupian" label="完成图片" prop="wanchengtupian">
          <file-upload
          tip="点击上传完成图片"
          action="file/upload"
          :limit="3"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.wanchengtupian" label="完成图片" prop="wanchengtupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.wanchengtupian.split(',')" :src="$base.url+item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="完成结果" prop="wanchengjieguo">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.wanchengjieguo" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.wanchengjieguo" label="完成结果" prop="wanchengjieguo">
                    <span v-html="ruleForm.wanchengjieguo"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  ...
  methods: {
	...
	this.ruleForm.wanchengjieguo = this.ruleForm.wanchengjieguo.replace(reg,'../../../ssmzw8q6/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `wanchengjieguo/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.wanchengjieguoCrossAddOrUpdateFlag = false;
						 this.parent.search();
